...
**Working capital for businesses, secured by a token's fee stream.**
...
A verified business hands its pool's **creator role** to Float and draws USDC against
the fees that role earns. While the loan is open, Float collects those fees straight
out of the pool. When the debt clears, the role goes back.
...
handle. A SOL-quoted pool would need all three, which is why v1 declines them.

Borrowing power comes from observed accrual instead:
...
- **There is no liquidation.** There is nothing liquid to seize. Float keeps collecting
...
| Path | What it is |
|---|---|
| `program/programs/float_credit/src/lib.rs` | The Solana program. Anchor 0.31.1. Nine instructions over five accounts. |
...
| `program/scripts/` | Devnet setup, the full loop, the security proof, and the borrower seeding. |
